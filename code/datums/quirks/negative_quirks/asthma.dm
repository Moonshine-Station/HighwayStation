/datum/quirk/item_quirk/asthma
	name = "Астма"
	desc = "У вас астма, воспалительное заболевание, при котором дыхательные пути сужаются! Будьте осторожны с дымом!"
...
/datum/quirk/item_quirk/asthma/add_unique(client/client_source)
	...
	give_item_to_holder(rescue_inhaler, list(LOCATION_BACKPACK, LOCATION_HANDS), flavour_text = "Вы можете использовать это, чтобы быстро облегчить симптомы астмы.")
	...
	if (holder_histamine)
		if (holder_histamine.overdosed) // uh oh!
			if (SPT_PROB(15, seconds_per_tick))
				to_chat(quirk_holder, span_boldwarning("Шея распухает, неумолимо сжимая дыхательное горло!"))
			adjust_inflammation(histamine_OD_inflammation * seconds_per_tick)
		else
			if (SPT_PROB(5, seconds_per_tick))
				to_chat(quirk_holder, span_warning("Хрипы становятся всё сильнее по мере того, как отекает шея..."))
...
/datum/quirk/item_quirk/asthma/proc/do_inflammation_change_feedback(difference)
	...
	if (difference > 0) // it decreased
		if (prob(1 * change_mult))
			...
			to_chat(quirk_holder, span_notice("Мокрота в горле вызывает приступ кашля!"))
		...
		if (prob(5 * change_mult))
			to_chat(quirk_holder, span_warning("Дыхательное горло сжимается..."))
