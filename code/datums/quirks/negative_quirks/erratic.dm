/datum/quirk/erratic
	...
	desc = "Ваше настроение качается, как маятник, из-за чего ваша личность может непредсказуемо меняться по малейшему поводу"
	icon = FA_ICON_MASKS_THEATER
	value = -3
	gain_text = span_danger("Вы чувствуете, как внутри всё шатается.") // say that again?
	lose_text = span_notice("Вы обретаете внутреннюю устойчивость.")
	medical_record_text = "У пациента наблюдается биполярное аффективное расстройство."
...
/datum/quirk/erratic/proc/randomize_personalities()
	...
	to_chat(quirk_holder, span_notice("Я чувствую себя... Другим."))
	announce_personality_change()
...
/datum/quirk/erratic/proc/announce_personality_change()
	...
	to_chat(quirk_holder, span_green("Теперь ваш характер: [english_list(new_personality)]."))
