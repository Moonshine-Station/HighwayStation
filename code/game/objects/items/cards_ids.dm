...
/obj/item/card/id/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	if(istype(tool, /obj/item/rupee))
		...
		if(!money_added)
			return ITEM_INTERACT_BLOCKING
		to_chat(user, span_notice("Вы вставляете содержимое на карту! Они исчезают в клубах блюспейс-дыма, пополняя счёт на [money_added] [MONEY_NAME_AUTOPURAL(money_added)] к привязанному аккаунту."))
		return ITEM_INTERACT_SUCCESS
	return NONE
...
/obj/item/card/id/proc/insert_money(obj/item/money, mob/user)
	...
	if(physical_currency)
		to_chat(user, span_notice("Вы запихиваете [money.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)]. Деньги исчезают в клубе блюспейс-дыма, пополняя счёт на [cash_money] [MONEY_NAME_AUTOPURAL(cash_money)]"))
	else
		to_chat(user, span_notice("Вы вставляете [money.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)], пополняя счёт на [cash_money] [MONEY_NAME_AUTOPURAL(cash_money)]"))
...
/obj/item/card/id/proc/set_new_account(mob/living/user)
	...
	to_chat(user, span_notice("Указанный аккаунт был привязан к этой ID-карте. Он содержит [account.account_balance] [MONEY_NAME_AUTOPURAL(account.account_balance)]"))
	return TRUE
...
/obj/item/card/id/click_alt(mob/living/user)
	...
	if(!registered_account.adjust_money(-amount_to_remove, "Система: Вывод средств"))
		...
		registered_account.bank_card_talk(span_warning("ОШИБКА: Для вывода средств с привязанного аккаунта требуется на [difference] [MONEY_NAME_AUTOPURAL(difference)] больше."), TRUE)
		return CLICK_ACTION_BLOCKING
...
/obj/item/card/id/proc/pay_debt(user)
	...
	if(amount_paid)
		var/message = span_notice("Вы заплатили [amount_to_pay] [MONEY_NAME_AUTOPURAL(amount_to_pay)] из [prev_debt] [MONEY_SYMBOL] вашего долга. Осталось выплатить [registered_account.account_debt] [MONEY_SYMBOL].")
		if(!registered_account.account_debt)
			message = span_nicegreen("Вы заплатили последние [amount_to_pay] [MONEY_NAME_AUTOPURAL(amount_to_pay)] из вашего долга, погасив его. Поздравляем!")
...
/obj/item/card/id/examine_more(mob/user)
	...
	if(registered_account)
		...
		if(registered_account.account_job)
			...
			if(D)
				. += "[D.account_holder] имеет баланс [D.account_balance] [MONEY_NAME_AUTOPURAL(D.account_balance)]"
